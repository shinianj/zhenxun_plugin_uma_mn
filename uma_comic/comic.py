...
async def download_img(id, url):
    img_path = os.path.join(os.path.dirname(__file__), 'comic/')
    ...
    if not os.path.exists(current_dir):
        ...
        logger.info(f'未检测到马娘漫画 uma_comic_{id}.jpg ，现已下载成功')
    else:
        logger.info(f'检测到马娘漫画 uma_comic_{id}.jpg 已存在，将不会重新下载')
...
async def get_comic_uma(uma_name_tmp):
    ...
    if not uma_name:
        return ''
    path = os.path.join(os.path.dirname(__file__), 'comic/')
    current_dir = os.path.join(os.path.dirname(__file__), f'comic_config.json')
