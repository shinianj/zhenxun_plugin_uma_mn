...
async def generate_url(sup_type):
    ...
    img_path = os.path.join(os.path.dirname(__file__), 'uma_support_chart/')
...
async def fix_img(img_dict, sup_type):
    ...
    for img_name in list(img_dict[sup_type]['img_data'].keys()):
        current_dir = os.path.join(os.path.join(os.path.dirname(__file__), 'uma_support_chart/'), f'{img_name}')
...
async def generate_img_tw(sup_type):
    try:
        ...
        return '请求超时，请重试'
    end_path = os.path.join(os.path.dirname(__file__), 'uma_support_chart/end_img_tw/')
    ...
    if not is_update:
        ...
        sv_help = f'{os.path.abspath(end_img_path)}' 
        async def n() -> str:
