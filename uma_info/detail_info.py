...
async def get_detail(uma_name, f_data):
    im = Image.open(os.path.join(os.path.dirname(__file__), f'res/base_data/background_data/{uma_name}.png'))
    # 制服图片就放在额外数据里
    current_dir = os.path.join(os.path.dirname(__file__), f'res/extra_data/uma_uniform/{uma_name}_uniform.png')
    uniform = str(f_data[uma_name]['uniform_img'])
    # 创建文件夹
    if not os.path.exists(os.path.join(os.path.dirname(__file__), f'res/extra_data/')):
        os.mkdir(os.path.join(os.path.dirname(__file__), f'res/extra_data/'))
    if not os.path.exists(os.path.join(os.path.dirname(__file__), f'res/extra_data/uma_uniform/')):
        os.mkdir(os.path.join(os.path.dirname(__file__), f'res/extra_data/uma_uniform/'))
    # 图片不存在就下载
