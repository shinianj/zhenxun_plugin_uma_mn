...
async def get_title(f_data):
    ...
    img_dir = os.path.join(os.path.dirname(__file__), 'uma_tasks/tasks_list.png')
    # 图片文件不存在就创建图片
    ...
    sv_help = f'{os.path.abspath(img_dir)}'
    async def n() -> str:
...
async def get_task_info(task_id, f_data):
    ...
    img_dir = os.path.join(os.path.dirname(__file__), f'uma_tasks/task_id_{task_id}.png')
    # 图片文件不存在就创建图片
    if not os.path.exists(img_dir):
...
async def create_img(is_title, info_data, filename_tmp):
    ...
    space = 5
    current_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), f'uma_tasks/simhei.ttf')
    ...
    save_dir = os.path.join(os.path.dirname(__file__), 'uma_tasks/')
