...
async def translate_news(news_id):
    ...
    try:
        ...
        if res_dict['detail']['image_big']:
            ...
            dir_path = os.path.join(os.path.dirname(__file__), 'umamusume_news/')
            if not os.path.exists(dir_path):
                os.mkdir(dir_path)
            save_dir = os.path.join(os.path.dirname(__file__), f'umamusume_news/news_img_{news_id}.jpg')
            if not os.path.exists(save_dir):
                response = requests.get(url=img_url, proxies=proxy)
                with open(save_dir, 'wb') as f:
                    f.write(response.content)
            head_img = str(os.path.dirname(f'umamusume/umamusume_news/news_img_{news_id}.jpg'))
